...
class ACSLV1(nn.Module):

    #def __init__(self, use_sigmoid=False,score_thr=0.7, json_file='./data/lvis_v0.5/annotations/lvis_v0.5_train.json', loss_weight=1.0):
    def __init__(self, use_sigmoid=False, score_thr=0.7, json_file='./data/lvis_v1/annotations/lvis_v1_train.json',
                 loss_weight=1.0):

        super(ACSLV1, self).__init__()
    ...
    def forward(self, cls_logits, labels, weight=None, avg_factor=None, reduction_override=None, **kwargs):
        ...
        for cls in unique_label:
            cls = cls.item()
            cls_inds = torch.nonzero(labels == cls, as_tuple=False).squeeze(1)

            if cls == 1203:
                # construct target vector for background samples
                target[cls_inds, 1203] = 1
